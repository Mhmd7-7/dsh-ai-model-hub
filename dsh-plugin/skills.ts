 ...
 */
export function registerModelHubSkill(
  ctx: Context,
  log: PluginLogger,
  skillFile: string = SKILL_FILE,
): void {
  ctx.inject(['skills'], (scope) => {
    ...
    const skills = (scope as unknown as { readonly skills?: SkillsService }).skills;
 ...
 */
export function createModelHubSkillProvider(
  log: PluginLogger,
  skillFile: string = SKILL_FILE,
): SkillProvider {
  let warningLogged = false;

  const read = async (): Promise<LoadedSkill | undefined> => {
    try {
      ...
      if (!warningLogged) {
        warningLogged = true;
        log.warn(
          `the bundled skill was not offered: ${describeError(error)}`,
        );
      }
      return undefined;
    }
  ...
  return {
    ...
    async get(candidate, options) {
      ...
      if (candidate.locator !== skillFile) return undefined;
      const skill = await read();
 ...
 */
function candidateFor(skill: LoadedSkill, skillFile: string): SkillCandidate {
  return {
    name: skill.name,
 ...
 */
function definitionFor(skill: LoadedSkill, skillFile: string): SkillDefinition {
  return {
    name: skill.name,
    description: skill.description,
    ...(skill.whenToUse === undefined ? {} : { whenToUse: skill.whenToUse }),
 ...
 */
function parseSkillFile(source: string, file: string): LoadedSkill {
  const match = /^---\r?\n([\s\S]*?)\r?\n---[ \t]*\r?\n?/.exec(source);
  const block = match?.[1];
