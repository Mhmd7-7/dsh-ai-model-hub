...
export function registerDiscoveryTools(
  ...
  ctx.tools.register(
    defineTool({
      name: 'list_models',
      description:
        'List the local AI models this machine offers, with their capabilities, live status, and resource requirements. ' +
      ...
      parameters: {
        capability: {
          type: 'string',
          description:
            'Only list models declaring this capability, e.g. text_to_image. Call list_capabilities to see the vocabulary.',
        },
        includeDisabled: {
          type: 'boolean',
      ...
      execute: async (args) => {
        ...
        const filtered =
          args.capability === undefined || args.capability.trim().length === 0
            ? views
            : views.filter((view) => view.model.capabilities.includes(args.capability as Capability));
        return {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List the media and text capabilities this machine can currently serve, which models provide each, and which capabilities have no model at all. ' +
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Report whether specific models are running, healthy, and available, including the reason when one is not. ' +
  ...
  ctx.tools.register(
    defineTool({
      name: 'list_artifacts',
      description:
        'List artifacts produced earlier â€” images, 3D meshes, audio, text â€” with their ids. ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          if (value.count === 0) return textBlock('No artifacts have been produced yet.');
          return textBlock(
            `${value.count} artifact(s), newest first:\n\n${value.artifacts.map((artifact) => `  ${artifact.description}\n    id: ${artifact.id}`).join('\n')}`,
          );
        },
      },
...
export function registerRoutingTool(
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Explain which model would be chosen for a capability and why, considering every candidate that was considered and rejected. ' +
