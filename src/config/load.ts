...
import type { ModelHub, ModelHubOptions } from '../hub.ts';
import { ModelHub as Hub } from '../hub.ts';

/** Where the catalog file was found. */
export interface LoadedCatalog {
  /** The absolute path of the file that was read. */
  readonly path: string;
  /** The raw parsed JSON, before validation. */
...
export interface LoadOptions {
  /**
   * An explicit file to read. When omitted, discovery walks upward from
   * {@link startDir} looking for a well-known filename.
   */
  readonly configPath?: string;
  /** Where discovery begins. Defaults to the process working directory. */
  readonly startDir?: string;
  /** Filenames to accept during discovery, in priority order. */
  readonly filenames?: readonly string[];
  /** Whether to search parent directories. Defaults to true. */
 ...
 */
export function findConfigDirectory(options: LoadOptions = {}): string | undefined {
  ...
  for (;;) {
    ...
    if (!searchParents) return undefined;
    const parent = resolve(current, '..');
    if (parent === current) return undefined;
    // A safety valve: never walk more than 20 levels.
    levels += 1;
    if (levels > 20) return undefined;
    current = parent;
  }
}
 ...
 */
export function loadCatalogConfig(options: LoadOptions = {}): LoadedCatalog {
  ...
  if (path === undefined) {
    ...
    throw new ModelHubError(
      'CONFIG_ERROR',
      `no model catalog found (searched for ${searched} from ${resolve(options.startDir ?? process.cwd())}). ` +
        'Create config/models.json, or call ModelHub with an explicit `config` object.',
      { configPath: options.configPath, startDir: options.startDir },
    );
  }
 ...
 */
function readCatalogAt(path: string): LoadedCatalog {
  let text: string;
  try {
  ...
  try {
    ...
    throw new ModelHubError(
      'CONFIG_ERROR',
      `model catalog at ${path} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      { path },
    );
  }
...
/**
 * Options for anchor-based catalog resolution.
 *
 * An *anchor* is a directory to start discovery from. Several are tried in
 * order, because the process that constructs the hub frequently has a working
 * directory that has nothing to do with the deployment: a long-running host
 * process is started wherever its launcher happened to be, long before any
 * workspace exists for it to serve.
 */
export interface AnchorLoadOptions {
  /**
   * An explicit catalog file. When set, it is the only thing tried: an
   * operator who names a file must be told that the file is wrong, never
   * quietly given a different catalog instead.
   */
  readonly configPath?: string;
  /** Directories to start discovery from, in priority order. */
  readonly anchors: readonly string[];
  /** Filenames to accept during discovery. */
  readonly filenames?: readonly string[];
}

/** A catalog located through {@link loadCatalogFromAnchors}. */
export interface AnchorLoadedCatalog extends LoadedCatalog {
  /** The anchor that matched, as resolved; the explicit path when one was given. */
  readonly anchor: string;
  /** Every anchor actually tried, in order, including the one that matched. */
 ...
 */
export function loadCatalogFromAnchors(options: AnchorLoadOptions): AnchorLoadedCatalog {
  ...
  for (const anchor of options.anchors) {
    ...
    if (found !== undefined) return { ...readCatalogAt(found), anchor: start, searched };
  }

  throw new ModelHubError(
    'CONFIG_ERROR',
    `no model catalog found. Searched upward from ${searched.length} anchor(s): ${searched.join(', ')}. ` +
      'Create models.json in one of those directories, or set the catalog path explicitly.',
    { anchors: searched },
  );
}
 ...
 */
export function loadHubFromDisk(
  options: Omit<ModelHubOptions, 'config'> & { readonly load?: LoadOptions } = {},
