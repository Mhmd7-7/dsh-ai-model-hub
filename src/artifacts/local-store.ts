...
import { isIoType } from '../catalog/capabilities.ts';
...
import type { Artifact, ArtifactStore, ArtifactWriteRequest } from './types.ts';

/** The current on-disk index format version. */
const INDEX_VERSION = 1;

/** The index document as stored on disk. */
 ...
 */
function chooseExtension(request: ArtifactWriteRequest): string {
  ...
  if (mime !== undefined) {
    ...
    if (suffix !== undefined && suffix.length > 0 && suffix.length <= 8 && /^[\w.+-]+$/.test(suffix)) {
      if (mime.startsWith('model/gltf-binary')) return '.glb';
      if (mime.startsWith('model/gltf+json')) return '.gltf';
...
/** Options for {@link LocalArtifactStore}. */
export interface LocalArtifactStoreOptions {
  /** Directory that holds `index.json` and the artifact files. Created on first write. */
...
export class LocalArtifactStore implements ArtifactStore {
  ...
  private async ensureLoaded(): Promise<void> {
    ...
    for (const entry of parsed['artifacts']) {
      ...
      if (artifact !== undefined) this.index.set(artifact.id, artifact);
    }
   ...
   */
  private parseIndexEntry(entry: unknown): Artifact | undefined {
    ...
    if (mimeType !== undefined) artifact.mimeType = mimeType;
    if (byteLength !== undefined) artifact.byteLength = byteLength;
    if (label !== undefined) artifact.label = label;
    if (producerModelId !== undefined) artifact.producerModelId = producerModelId;
    return artifact;
  }
  ...
  private async persistIndex(): Promise<void> {
    const document: ArtifactIndex = { version: INDEX_VERSION, artifacts: [...this.index.values()] };
   ...
   */
  async put(request: ArtifactWriteRequest): Promise<Artifact> {
    const hasBytes = request.bytes !== undefined;
    const hasText = request.text !== undefined;
    if (hasBytes === hasText) {
      throw new ModelHubError(
        'ARTIFACT_ERROR',
        'an artifact write must supply exactly one of `bytes` or `text`',
        { type: request.type, hasBytes, hasText },
      );
    }
    const payload =
      request.bytes !== undefined ? Buffer.from(request.bytes) : Buffer.from(request.text ?? '', 'utf8');
    ...
    return this.serialize(async () => {
      ...
      if (mimeType !== undefined) artifact.mimeType = mimeType;
      if (request.label !== undefined) artifact.label = request.label;
      if (request.producerModelId !== undefined) artifact.producerModelId = request.producerModelId;
   ...
   */
  private mintId(request: ArtifactWriteRequest, payload: Buffer, extension: string): string {
   ...
   */
  async read(id: string): Promise<{ artifact: Artifact; bytes: Uint8Array }> {
   ...
   */
  async resolvePath(id: string): Promise<{ artifact: Artifact; path: string }> {
    await this.ensureLoaded();
    ...
    if (path === undefined) {
      throw new ModelHubError(
        'ARTIFACT_ERROR',
        `artifact ${id} has a non-file URI ("${artifact.uri}") that this store cannot resolve`,
        { artifactId: id, uri: artifact.uri },
      );
    }
    ...
    if (!path.startsWith(contentRoot)) {
      throw new ModelHubError(
        'ARTIFACT_ERROR',
        `artifact ${id} points outside the store root; refusing to expose ${path}`,
        { artifactId: id, path, contentRoot },
      );
    }
    try {
      await stat(path);
   ...
   */
  async list(limit = 100): Promise<Artifact[]> {
    await this.ensureLoaded();
   ...
   */
  async findOrphans(): Promise<readonly string[]> {
    await this.ensureLoaded();
   ...
   */
  async findUnreferencedFiles(): Promise<readonly string[]> {
    await this.ensureLoaded();
    const referenced = new Set<string>();
    for (const artifact of this.index.values()) {
      const path = fileUriToPath(artifact.uri);
      if (path !== undefined) referenced.add(resolve(path));
    }
    let entries: string[];
    try {
    ...
    for (const entry of entries) {
      ...
      if (!referenced.has(absolute) && extname(entry) !== '.tmp') unreferenced.push(absolute);
    }
    return unreferenced;
  }
}
