...
const DISCOVERED_TAGS: readonly string[] = ['local', 'discovered', COMFYUI_ENGINE];
...
export type WeightKind = keyof typeof WEIGHT_FIELDS;

/** A node class's capability signal. */
interface CapabilitySignal {
  /** Matches a node class name, case-insensitively, anywhere inside it. */
  readonly pattern: RegExp;
  /** The capability the node's presence proves the engine can serve. */
...
export interface ComfyWeightFile {
  /** The filename exactly as ComfyUI enumerated it — the value a graph must use. */
  readonly filename: string;
  /** Which loader kind enumerated it. */
  readonly kind: WeightKind;
}

/** What `/object_info` revealed about this ComfyUI install. */
export interface ComfyIntrospection {
  /** Every node class the server has loaded. */
  readonly nodeClasses: readonly string[];
  /** Every weight file found, in enumeration order, deduplicated by kind and filename. */
  readonly files: readonly ComfyWeightFile[];
  /** Capabilities the installed node packs prove, in the order the table declares them. */
  readonly capabilities: readonly Capability[];
  /** The capability signals that fired, for diagnostics. */
 ...
 */
export function parseComfyObjectInfo(raw: unknown): ComfyIntrospection {
  const nodeClasses: string[] = [];
  const files: ComfyWeightFile[] = [];
  const seen = new Set<string>();
  const capabilities: Capability[] = [];
  const signals: string[] = [];

  if (!isRecordLike(raw)) return { nodeClasses, files, capabilities, signals };

  for (const className of Object.keys(raw)) {
    nodeClasses.push(className);

    const required = isRecordLike(raw[className]) ? (raw[className] as Record<string, unknown>)['input'] : undefined;
 ...
 */
function weightKindForField(fieldName: string): WeightKind | undefined {
  for (const [kind, field] of Object.entries(WEIGHT_FIELDS) as [WeightKind, string][]) {
    if (fieldName === field) return kind;
  }
  ...
  if (fieldName.endsWith('lora_name')) return 'lora';
  return undefined;
}
 ...
 */
function fileOptionsIn(fieldSpec: unknown): string[] {
  if (!Array.isArray(fieldSpec)) return [];
  const candidates: unknown = Array.isArray(fieldSpec[0]) ? fieldSpec[0] : fieldSpec;
  if (!Array.isArray(candidates)) return [];
  const options: string[] = [];
  for (const option of candidates) {
    if (typeof option === 'string' && option.trim().length > 0) options.push(option);
  }
  return options;
}
 ...
 */
export function capabilitiesForComfyModel(introspection: ComfyIntrospection): Capability[] {
  const capabilities: Capability[] = ['text_to_image'];
 ...
 */
export function estimateComfyVram(filename: string): number {
  for (const rule of VRAM_ESTIMATE_PATTERNS) {
    if (rule.pattern.test(filename)) return rule.vramGb;
  }
...
export function describeIntrospection(introspection: ComfyIntrospection): string {
  const counts = new Map<WeightKind, number>();
  for (const file of introspection.files) counts.set(file.kind, (counts.get(file.kind) ?? 0) + 1);
