...
import { fetchJson, isRecordLike, readString, slugifyModelId, stableDigest } from './http.ts';
...
export const A1111_ENGINES: readonly string[] = ['a1111', 'automatic1111', 'forge', 'sd_webui', 'stable_diffusion_webui'];
...
const DISCOVERED_TAGS: readonly string[] = ['local', 'discovered', A1111_ENGINE];
 ...
 */
const VRAM_ESTIMATE_PATTERNS: readonly { readonly pattern: RegExp; readonly vramGb: number }[] = [
...
/** One installed checkpoint as reported by `GET /sdapi/v1/sd-models`. */
export interface A1111ModelSummary {
  /** The display title, which is also the value the API accepts for a switch. */
  readonly title: string;
  /** The engine's own short model name, kept for diagnostics. */
  readonly modelName?: string;
  /** Content hash, when the engine reported one. */
  readonly hash?: string;
  /** The checkpoint's filename as it exists on disk. */
  readonly filename?: string;
}

/** The engine-wide facts read once per host. */
export interface A1111EngineFacts {
  /** Sampler names this build accepts. */
  readonly samplers: readonly string[];
  /** The checkpoint the engine reports as loaded, when it said. */
  readonly loadedCheckpoint?: string;
}
 ...
 */
export function parseA1111Models(raw: unknown): A1111ModelSummary[] {
  if (!Array.isArray(raw)) return [];
  const models: A1111ModelSummary[] = [];
  for (const entry of raw) addSummary(models, entry);
  return models;
}
 ...
 */
function addSummary(models: A1111ModelSummary[], entry: unknown): void {
 ...
 */
export function parseA1111Samplers(raw: unknown): string[] {
  if (!Array.isArray(raw)) return [];
  const samplers: string[] = [];
  for (const entry of raw) addSampler(samplers, entry);
  return samplers;
}
 ...
 * @param entry - one raw array element.
 */
function addSampler(samplers: string[], entry: unknown): void {
  const name = readString(entry, 'name') ?? readString(entry, 'aliases');
  if (name !== undefined && !samplers.includes(name)) samplers.push(name);
}
 ...
 */
export function parseA1111Options(raw: unknown): string | undefined {
 ...
 */
export const A1111_CAPABILITIES: readonly Capability[] = ['text_to_image', 'image_to_image'];
 ...
 */
export function estimateA1111Vram(names: readonly (string | undefined)[]): number {
  const haystack = names.filter((name): name is string => name !== undefined).join(' ');
  if (haystack.trim().length === 0) return DEFAULT_VRAM_GB;
  for (const rule of VRAM_ESTIMATE_PATTERNS) {
    if (rule.pattern.test(haystack)) return rule.vramGb;
  }
 ...
 */
export function mapA1111Model(
  summary: A1111ModelSummary,
  host: ModelHost,
  facts: A1111EngineFacts,
): ModelDescriptor {
 ...
 */
function isLoadedCheckpoint(summary: A1111ModelSummary, loaded: string): boolean {
  const candidates = [summary.title, summary.modelName, summary.filename].filter(
 ...
 */
function modelTypeFor(): ModelType {
 ...
 * @returns the note.
 */
function describeA1111Model(
  summary: A1111ModelSummary,
  facts: A1111EngineFacts,
  loaded: boolean,
  vramGb: number,
): string {
  const factsOut: string[] = ['Discovered from the A1111/Forge API; not listed in models.json.'];
  if (loaded) factsOut.push('Currently loaded, so it serves without a checkpoint switch.');
  if (summary.filename !== undefined) factsOut.push(`File: ${summary.filename}.`);
  if (facts.samplers.length > 0) factsOut.push(`${facts.samplers.length} sampler(s) available.`);
 ...
 */
export function createA1111Discoverer(
 ...
 */
function optionalLoaded(loaded: string | undefined): { readonly loadedCheckpoint?: string } {
