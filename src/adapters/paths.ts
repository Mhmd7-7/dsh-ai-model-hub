...
/** Which base a catalog-written path was resolved against. */
export type AdapterPathOrigin =
  /** The path was already absolute; no base was consulted. */
  | 'absolute'
  /** The catalog's own directory — the normal case. */
  | 'catalog'
  /** The process working directory, because the hub has no catalog file. */
  | 'working-directory';

/** A path written in a catalog, resolved to something that can be read. */
export interface ResolvedAdapterPath {
  /** The absolute path to read. */
  readonly absolute: string;
  /** The directory {@link absolute} was resolved against; absent for an absolute path. */
  readonly base?: string;
  /** Which base was used, so a message can state the rule that applied. */
  readonly origin: AdapterPathOrigin;
}
 ...
 */
export function resolveAdapterPath(path: string, catalogDir: string | undefined): ResolvedAdapterPath {
  if (isAbsolute(path)) return { absolute: path, origin: 'absolute' };
 ...
 */
export function describeAdapterPath(resolved: ResolvedAdapterPath): string {
