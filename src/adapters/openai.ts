 ...
 */
const CHAT_CAPABILITIES: readonly Capability[] = ['text_to_text', 'image_understanding'];

/** Generation settings resolved from per-call options, then adapter config. */
interface ChatSettings {
  /** The checkpoint name to send, when one is configured. */
  ...
  /** Deterministic sampling seed, for engines that support one. */
  readonly seed: number | undefined;
  /** Deadline for one request, in milliseconds. */
  readonly timeoutMs: number;
  /** Operator-supplied keys merged into the request body verbatim. */
  readonly extraBody: Readonly<Record<string, unknown>> | undefined;
}

/** A text fragment of a chat message. */
interface TextPart {
  readonly type: 'text';
  readonly text: string;
}

/** An inline image fragment of a chat message. */
interface ImagePart {
  readonly type: 'image_url';
  readonly image_url: { readonly url: string };
}

/** One fragment of a multimodal user message. */
 ...
 */
function textSetting(invocation: AdapterInvocation, key: string): string | undefined {
 ...
 */
function numberSetting(invocation: AdapterInvocation, key: string): number | undefined {
 ...
 */
function settingsFor(invocation: AdapterInvocation): ChatSettings {
 ...
 */
function requestUrl(model: ResolvedModel): string | undefined {
...
async function buildMessages(
  ...
  if (settings.system !== undefined) messages.push({ role: 'system', content: settings.system });
  ...
  for (const artifact of invocation.inputs) {
    if (artifact.type === 'text' || artifact.type === 'json') {
      ...
      const body =
        decoded.length > MAX_INLINE_TEXT_CHARS
    ...
    if (artifact.type === 'image') {
      if (invocation.capability !== 'image_understanding') {
        throw new ModelHubError(
          ...
          `the openai_compatible adapter cannot feed an image to capability "${invocation.capability}"`,
          { modelId: invocation.model.id, capability: invocation.capability, artifactId: artifact.id },
        );
      }
      const { bytes } = await invocation.artifacts.read(artifact.id);
      if (bytes.byteLength > MAX_INLINE_IMAGE_BYTES) {
        throw new ModelHubError(
          'ARTIFACT_ERROR',
          `input image "${artifact.id}" is ${bytes.byteLength} bytes, above the ${MAX_INLINE_IMAGE_BYTES}-byte inline limit`,
          { artifactId: artifact.id, byteLength: bytes.byteLength },
        );
      }
    ...
    throw new ModelHubError(
      'UNSUPPORTED_OPERATION',
      `the openai_compatible adapter cannot consume a "${artifact.type}" input`,
      { modelId: invocation.model.id, artifactId: artifact.id, artifactType: artifact.type },
    );
  }
  ...
  if (text.trim().length === 0) {
    throw new ModelHubError(
      'INVOCATION_FAILED',
      `capability "${invocation.capability}" requires a \`prompt\` or a text input artifact and neither was supplied`,
  ...
  else messages.push({ role: 'user', content: [{ type: 'text', text }, ...images] });
...
function extractText(
  payload: unknown,
): { readonly text: string; readonly finishReason: string | undefined } | undefined {
  ...
  if (typeof legacy === 'string') return { text: legacy, finishReason };
  ...
  if (Array.isArray(content)) {
    const joined = content
      ...
      .join('');
    if (joined.length > 0) return { text: joined, finishReason };
  }
  return undefined;
}
...
function transportError(
  ...
  invocation: AdapterInvocation,
  url: string,
  options: { readonly callerAborted: boolean; readonly timedOut: boolean; readonly timeoutMs: number },
): ModelHubError {
  ...
  if (options.timedOut) {
    return new ModelHubError(
      'INVOCATION_TIMEOUT',
      `"${modelId}" exceeded its ${options.timeoutMs} ms budget at ${url}`,
      { modelId, endpoint: url, timeoutMs: options.timeoutMs },
    );
  }
 ...
 */
export function createOpenAiCompatibleAdapter(): ModelAdapter {
  return {
    ...
    supports(model: ResolvedModel): { ok: true } | { ok: false; reason: string } {
      if (model.runtime.endpoint === undefined) {
        return {
          ok: false,
          reason:
            'the openai_compatible adapter requires `runtime.endpoint` (e.g. http://127.0.0.1:11434)',
        };
      }
      ...
      if (model.capabilities.length > 0 && unsupported.length === model.capabilities.length) {
        return {
          ok: false,
          reason:
            `it declares only ${unsupported.join(', ')}, and this adapter serves ${CHAT_CAPABILITIES.join(', ')}. ` +
    ...
    async invoke(invocation: AdapterInvocation): Promise<AdapterOutput> {
      ...
      if (!CHAT_CAPABILITIES.includes(invocation.capability)) {
        throw new ModelHubError(
          'UNSUPPORTED_OPERATION',
          `the openai_compatible adapter does not implement capability "${invocation.capability}"`,
          { modelId: model.id, capability: invocation.capability, supported: [...CHAT_CAPABILITIES] },
        );
      }
      ...
      if (url === undefined) {
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" has no usable runtime.endpoint for the openai_compatible adapter`,
          { modelId: model.id, endpoint: model.runtime.endpoint },
        );
      }
      ...
      if (settings.model !== undefined) body['model'] = settings.model;
      ...
      if (settings.seed !== undefined) body['seed'] = settings.seed;
      ...
      if (settings.extraBody !== undefined) Object.assign(body, settings.extraBody);

      // One controller, two abort sources, so the reason a request ended is
      // never ambiguous to the classification below.
      const controller = new AbortController();
      let timedOut = false;
      const callerAborted = (): boolean => invocation.signal.aborted;
      ...
      try {
        ...
        if (!response.ok) {
          throw new ModelHubError(
            'INVOCATION_FAILED',
            `${url} returned HTTP ${response.status}: ${raw.trim().slice(0, MAX_ERROR_BODY_CHARS) || '(empty body)'}`,
            {
              modelId: model.id,
              endpoint: url,
        ...
        try {
          ...
          throw new ModelHubError(
            'INVOCATION_FAILED',
            `${url} returned a body that is not JSON: ${error instanceof Error ? error.message : String(error)}`,
            { modelId: model.id, endpoint: url, reason: 'malformed-body' },
          );
        }
        ...
        if (extracted === undefined) {
          throw new ModelHubError(
            'INVOCATION_FAILED',
            `${url} returned no message content: ${raw.trim().slice(0, MAX_ERROR_BODY_CHARS) || '(empty body)'}`,
            { modelId: model.id, endpoint: url, reason: 'empty-completion' },
          );
        }
        ...
        if (servedModel !== undefined) value['model'] = servedModel;
        if (promptTokens !== undefined) value['promptTokens'] = promptTokens;
        if (completionTokens !== undefined) value['completionTokens'] = completionTokens;
