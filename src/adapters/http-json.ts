...
/** Generation settings, resolved from per-call options over adapter config. */
 ...
 */
function settingsFor(invocation: AdapterInvocation): ImageSettings {
  ...
  return {
    ...
    ...(numberSetting(options, config, 'steps') === undefined
      ? {}
      : { steps: numberSetting(options, config, 'steps') as number }),
    ...(numberSetting(options, config, 'cfgScale') === undefined
      ? {}
      : { cfgScale: numberSetting(options, config, 'cfgScale') as number }),
    ...
    ...(numberSetting(options, config, 'seed') === undefined
      ? {}
      : { seed: numberSetting(options, config, 'seed') as number }),
    ...
    ...(stringSetting(options, config, 'model') === undefined
      ? {}
      : { checkpoint: stringSetting(options, config, 'model') as string }),
    timeoutMs: Math.max(1, numberSetting(options, config, 'timeoutMs') ?? DEFAULT_TIMEOUT_MS),
    ...(recordSetting(config, 'extraBody') === undefined
      ? {}
      : { extraBody: recordSetting(config, 'extraBody') as Readonly<Record<string, unknown>> }),
  };
}
 ...
 */
function requestUrl(model: ResolvedModel, capability: Capability): string | undefined {
  ...
  const path =
    capability === 'image_to_image' && (configured === undefined || configured === DEFAULT_TXT2IMG_PATH)
 ...
 */
export function createHttpJsonAdapter(): ModelAdapter {
  return {
    kind: 'http_json',
    ...
    async invoke(invocation: AdapterInvocation): Promise<AdapterOutput> {
      ...
      if (!IMAGE_CAPABILITIES.includes(capability)) {
        throw new ModelHubError(
          ...
          `the http_json adapter does not implement capability "${capability}"`,
          { modelId: model.id, capability, supported: [...IMAGE_CAPABILITIES] },
        );
      }
      ...
      if (url === undefined) {
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" has no usable runtime.endpoint for the http_json adapter`,
      ...
      const body: Record<string, unknown> = { prompt: settings.prompt };
      if (settings.negativePrompt !== undefined) body['negative_prompt'] = settings.negativePrompt;
      if (settings.steps !== undefined) body['steps'] = settings.steps;
      if (settings.cfgScale !== undefined) body['cfg_scale'] = settings.cfgScale;
      if (settings.sampler !== undefined) body['sampler_name'] = settings.sampler;
      if (settings.seed !== undefined) body['seed'] = settings.seed;
      if (settings.width !== undefined) body['width'] = settings.width;
      if (settings.height !== undefined) body['height'] = settings.height;
      if (settings.checkpoint !== undefined) body['override_settings_restore_afterwards'] = true;
      ...
      if (capability === 'image_to_image') {
        ...
        if (source === undefined) {
          throw new ModelHubError(
            ...
            { modelId: model.id, providedTypes: invocation.inputs.map((artifact) => artifact.type) },
          );
        }
      ...
      try {
        ...
        if (timedOut) {
          throw new ModelHubError(
            'INVOCATION_TIMEOUT',
            `model "${model.id}" did not answer ${url} within ${settings.timeoutMs} ms`,
            { modelId: model.id, url, timeoutMs: settings.timeoutMs },
          );
        }
        ...
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" could not reach ${url}: ${error instanceof Error ? error.message : String(error)}`,
          { modelId: model.id, url },
        );
      } finally {
        clearTimeout(timer);
      ...
      if (!response.ok) {
        ...
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" answered ${response.status} ${response.statusText} from ${url}: ${detail}`,
          { modelId: model.id, url, status: response.status },
        );
      }
      ...
      try {
        ...
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" returned a body that is not JSON: ${error instanceof Error ? error.message : String(error)}`,
          { modelId: model.id, url },
        );
      }
      ...
      if (images.length === 0) {
        ...
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" returned no images from ${url}. Expected { images: [...] }; ` +
            'if this endpoint serves chat completions, it is not an image engine.',
          { modelId: model.id, url, keys: Object.keys(payload as Record<string, unknown>).slice(0, 10) },
        );
      }
      ...
      for (const [index, encoded] of images.entries()) {
        ...
        if (bytes.length === 0) {
          throw new ModelHubError(
            'INVOCATION_FAILED',
            `model "${model.id}" returned image ${index} as base64 that decoded to zero bytes`,
            { modelId: model.id, url, index },
          );
        }
