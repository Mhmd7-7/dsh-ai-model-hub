...
/** How the adapter reaches the engine. */
type ThreeDProtocol = 'gradio' | 'http_json';

/** The outcome of reading one engine response. */
type ResponseRead =
  | { readonly ok: true; readonly status: number; readonly body: string; readonly contentType: string | undefined }
  | {
      readonly ok: false;
...
/** One generation step, fully resolved from adapter config. */
interface ResolvedStep {
  /** The step's 0-based position, used in `$N` bindings and diagnostics. */
  readonly index: number;
  /** The queue or route name, e.g. `image_to_3d`. */
  readonly apiName: string | undefined;
  /** Overrides the endpoint for this step only. */
  readonly endpoint: string;
  /** Argument names bound to earlier results, to the input, or to literal values. */
  readonly bind: Readonly<Record<string, JsonValue>>;
  /** Literal arguments appended after the bound ones. */
  readonly extraArgs: readonly JsonValue[];
  /** Whether this step's response is the mesh. */
  readonly primary: boolean;
  /** The format this step is expected to produce, when declared. */
  readonly resultFormat: ThreeDFormat | undefined;
  /** HTTP method for the `http_json` transport. */
  readonly method: string;
  /** Where in the response the result lives, e.g. `data` or `0`. */
  readonly resultAt: string | undefined;
}

/** Everything the adapter needs, resolved once per invocation. */
interface ResolvedConfig {
  readonly protocol: ThreeDProtocol;
  readonly endpoint: string;
  readonly steps: readonly ResolvedStep[];
  readonly inputMode: 'data_uri' | 'base64' | 'path';
  readonly filePath: string | undefined;
  /** Route a liveness probe should use for the `http_json` transport. */
  readonly healthPath: string | undefined;
  readonly timeoutMs: number;
  readonly requestTimeoutMs: number;
  readonly extraBody: Readonly<Record<string, JsonValue>>;
  readonly generationParameters: Readonly<Record<string, JsonValue>>;
  readonly previewIndex: number | undefined;
}

/** The mesh a step produced, plus everything measured about it. */
interface MeshPayload {
  readonly bytes: Uint8Array;
  readonly format: ThreeDFormat | undefined;
  readonly warning: string | undefined;
  /** The format the step claimed, if any — used when sniffing is inconclusive. */
  readonly declaredFormat: ThreeDFormat | undefined;
}
 ...
 */
function resolveConfigSync(model: ResolvedModel): ResolvedConfig {
...
async function readStepsFile(
  ...
  if (config['steps'] !== undefined) {
    throw configError(
      `${where}: declare either adapterConfig.steps or adapterConfig.stepsPath, not both`,
      { modelId: model.id },
    );
  }
  ...
  const merged: Record<string, unknown> = { ...config, steps };
 ...
 */
function requireProtocol(config: Readonly<Record<string, unknown>>, where: string): ThreeDProtocol {
  const value = optionalString(config, 'protocol') ?? 'gradio';
  if (value !== 'gradio' && value !== 'http_json') {
    throw configError(
      `${where}: adapterConfig.protocol must be "gradio" or "http_json" (got "${value}"). ` +
        'Gradio app servers (TRELLIS, Hunyuan3D, Stable Fast 3D, TripoSR) use "gradio"; ' +
        'FastAPI/Flask servers that take a JSON body use "http_json".',
...
function requireEndpoint(
  ...
  if (raw === undefined || raw.trim().length === 0) {
    throw configError(
      `${where}: no endpoint. Set adapterConfig.endpoint or runtime.endpoint to the engine's base URL, ` +
...
function resolveSteps(
  ...
  const steps: ResolvedStep[] = entries.map((entry, index) => {
    ...
    if (formatValue !== undefined && resultFormat === undefined) {
      throw configError(
        `${stepPath}: resultFormat "${formatValue}" is not a 3D format this hub knows (glb, gltf, obj, stl, ply)`,
        { step: index, resultFormat: formatValue },
      );
    }
    ...
    if (protocol === 'gradio' && apiName === undefined && config['stepsPath'] === undefined) {
      ...
      throw configError(
        `${stepPath}: the gradio protocol needs an apiName for every step — the name Gradio shows in the app's ` +
          'API docs, e.g. "image_to_3d". Without it there is no route to submit the job to.',
        { step: index, field: 'apiName' },
      );
    }
 ...
 */
function validateBindings(bind: Readonly<Record<string, JsonValue>>, stepIndex: number, where: string): void {
  for (const [name, value] of Object.entries(bind)) {
    ...
    if (expression === '$input' || expression === '$image') continue;
    ...
    if (!expression.startsWith('$')) {
      throw configError(
        `${where}: bind.${name} must start with "$" — use $input for the request's image, $param:<key> for a ` +
          `request option, or $N[.M] for an earlier step's output. Got "${expression}".`,
        { step: stepIndex, argument: name, expression },
      );
    }
    ...
    if (!Number.isInteger(referenced) || referenced < 0) {
      throw configError(
        `${where}: bind.${name} names step "${stepPart}", which is not a step index`,
        { step: stepIndex, argument: name, expression },
      );
    }
    if (referenced >= stepIndex) {
      throw configError(
        `${where}: bind.${name} names step ${referenced}, which has not run yet (this is step ${stepIndex}). ` +
          'A step can only bind to results from earlier steps.',
        { step: stepIndex, argument: name, expression },
      );
    }
    if (rest.length > 0 && !isWellFormedPath(rest.join('.'))) {
      throw configError(
        `${where}: bind.${name} indexes into step ${referenced}'s result with "${rest.join('.')}", ` +
          'which is not a property name or array index',
        { step: stepIndex, argument: name, expression },
      );
    }
  }
 ...
 */
function readBindings(raw: unknown, where: string): Readonly<Record<string, JsonValue>> {
 ...
 */
function readJsonArray(raw: unknown, field: string, where: string): readonly JsonValue[] {
  if (raw === undefined) return [];
 ...
 */
function optionalNumber(config: Readonly<Record<string, unknown>>, key: string): number | undefined {
  const value = config[key];
 ...
 * @returns the number, or `undefined`.
 */
function positiveNumber(config: Readonly<Record<string, unknown>>, key: string): number | undefined {
 ...
 */
function configError(message: string, details: Record<string, unknown> = {}): ModelHubError {
 ...
 */
export function createThreeDAdapter(): ModelAdapter {
  return {
    kind: 'three_d',
    ...
    async invoke(invocation: AdapterInvocation): Promise<AdapterOutput> {
      ...
      if (!THREE_D_CAPABILITIES.includes(invocation.capability)) {
        throw new ModelHubError(
          ...
          `the three_d adapter serves ${THREE_D_CAPABILITIES.join(' and ')}, not "${invocation.capability}"`,
          { modelId: model.id, capability: invocation.capability },
        );
      }
      ...
      log.info(
        `three_d: ${invocation.capability} via ${config.protocol} at ${config.endpoint} (${config.steps.length} step(s))`,
        { modelId: model.id },
      );
      ...
      const onOuterAbort = (): void => cancel.abort();
      if (invocation.signal.aborted) cancel.abort();
      else invocation.signal.addEventListener('abort', onOuterAbort, { once: true });
      const scoped: AdapterInvocation = { ...invocation, signal: cancel.signal };

      const run = async (): Promise<MeshPayload | undefined> => {
        ...
        for (const step of config.steps) {
          ...
          if (step.primary) {
            ...
            if (payload !== undefined) mesh = payload;
          }
        }
        return mesh;
      };
      ...
      if (mesh === undefined) {
        throw new ModelHubError(
          'INVOCATION_FAILED',
          `model "${model.id}" completed without producing a 3D asset. ` +
            ...
            'generated a model.',
          { modelId: model.id, endpoint: config.endpoint, steps: config.steps.length },
        );
      }
      ...
      if (preview !== undefined) artifacts.push(preview);
      ...
      return {
        ...
        value: {
          ...
          ...(primary.metadata['vertexCount'] === undefined
            ? {}
            : { vertexCount: primary.metadata['vertexCount'] as JsonValue }),
          ...(primary.metadata['triangleCount'] === undefined
            ? {}
            : { triangleCount: primary.metadata['triangleCount'] as JsonValue }),
 ...
 */
function requireInputImage(invocation: AdapterInvocation): Artifact {
  const image = invocation.inputs.find((artifact) => artifact.type === 'image');
  if (image === undefined) {
    throw new ModelHubError(
      'INVOCATION_FAILED',
      `"${invocation.capability}" needs an input artifact of type \`image\`; ` +
        `this request supplied ${invocation.inputs.length === 0 ? 'none' : invocation.inputs.map((artifact) => artifact.type).join(', ')}. ` +
        'Pass the image artifact id in `inputs`, or generate one first with text_to_image.',
      { modelId: invocation.model.id, kind: invocation.capability },
    );
  }
  return image;
}
 ...
 */
function invocationTimeout(invocation: AdapterInvocation, config: ResolvedConfig): number {
...
async function buildArguments(
  ...
  for (const [name, bound] of Object.entries(step.bind)) {
    ...
    args.push(
      typeof bound === 'string' && bound.startsWith('$')
  ...
  for (const extra of step.extraArgs) args.push(extra);
  ...
  if (Object.keys(step.bind).length === 0 && step.index > 0) {
    ...
    for (const value of previous) args.push(toJsonValue(value, step.index));
  }
...
async function resolveBinding(
  ...
  if (expression.startsWith('$param:')) {
    ...
    if (value === undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `argument "${name}" is bound to the request option "${key}", which was supplied neither by the caller ` +
          'nor as a default in the model\'s adapterConfig.generationParameters',
  ...
  if (!expression.startsWith('$')) {
    throw configError(
      `binding for argument "${name}" must start with "$" ($input, $param:<key>, or $N[.M]); got "${expression}". ` +
        'A literal argument value does not need a binding at all — put it in `bind` directly.',
  ...
  if (!Number.isInteger(stepIndex) || stepIndex < 0) {
    throw configError(
      `binding for argument "${name}" names step "${stepPart}", which is not a step index`,
      { argument: name, expression },
    );
  }
  ...
  if (stepResults === undefined) {
    throw configError(
      `binding for argument "${name}" names step ${stepIndex}, which has not run yet ` +
        `(this step is number ${results.length})`,
      { argument: name, expression, availableSteps: results.length },
    );
  }
  ...
  if (value === undefined) {
    throw new ModelHubError(
      'INVOCATION_FAILED',
      `binding for argument "${name}" resolved "${expression}" to nothing in step ${stepIndex}'s response ` +
        `(that step returned ${stepResults.length} output(s))`,
      { argument: name, expression, stepOutputs: stepResults.length },
    );
  }
 ...
 */
function toJsonValue(value: unknown, stepIndex: number): JsonValue {
  if (isLosslessJson(value)) return value;
  throw new ModelHubError(
    'INVOCATION_FAILED',
    `a value from step ${stepIndex < 0 ? 'the request' : stepIndex} cannot be passed to the engine: ` +
      'only JSON-serializable data can cross an engine boundary',
    { stepIndex },
  );
}
 ...
 */
async function encodeInputImage(
  source: Artifact,
  invocation: AdapterInvocation,
  config: ResolvedConfig,
): Promise<string> {
  const { path } = await invocation.artifacts.resolvePath(source.id);
  if (config.inputMode === 'path') return path;
...
async function runStep(
  ...
  if (config.protocol === 'gradio') {
    if (step.apiName === undefined) {
      throw configError(
        `step ${step.index}: the gradio protocol needs an apiName (the name Gradio shows in its API docs, e.g. "image_to_3d")`,
        { step: step.index },
      );
    }
...
async function runGradioStep(
  ...
  for (const prefix of prefixes) {
    ...
    if (!submitted.ok) {
      ...
      if (submitted.status === 404) continue;
    ...
    if (eventId === undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine at ${submitUrl} accepted the request but returned no event id, so its result cannot be collected. ` +
          `Response began: ${bounded(submitted.body)}`,
        { modelId: invocation.model.id, url: submitUrl },
      );
    }
    ...
    if (!streamed.ok) throw engineError(resultUrl, streamed.reason, invocation.model.id);

    const events = parseSse(streamed.body);
    if (events.error !== undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine reported an error while generating: ${bounded(events.error)}`,
        { modelId: invocation.model.id, step: step.index },
      );
    }
    if (events.complete === undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine's result stream for "${apiName}" ended without a completion event`,
        { modelId: invocation.model.id, step: step.index, events: events.seen },
      );
    }
    let parsed: unknown;
    try {
      parsed = JSON.parse(events.complete) as unknown;
    } catch {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine's completion event for "${apiName}" was not JSON: ${bounded(events.complete)}`,
        { modelId: invocation.model.id, step: step.index },
      );
    }
    const outputs = isRecord(parsed) && Array.isArray(parsed['data']) ? parsed['data'] : [parsed];
    if (step.resultAt !== undefined) return [pathGet(outputs, step.resultAt) ?? pathGet(parsed, step.resultAt)];
    return outputs;
  }
 ...
 */
async function runHttpJsonStep(
  step: ResolvedStep,
  args: readonly JsonValue[],
  config: ResolvedConfig,
  invocation: AdapterInvocation,
  ...
  if (!read.ok) throw engineError(url, read.reason, invocation.model.id);
  ...
  try {
    ...
    throw new ModelHubError(
      'INVOCATION_FAILED',
      `the engine at ${url} answered with something that is not JSON: ${bounded(read.body)}`,
      { modelId: invocation.model.id, url },
    );
  }
  ...
  if (step.resultAt !== undefined) {
    ...
    if (addressed === undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `resultAt "${step.resultAt}" matched nothing in the engine's response from ${url}. ` +
          `Response keys: ${Object.keys(isRecord(parsed) ? parsed : {}).join(', ') || '(none)'}`,
        { modelId: invocation.model.id, url, resultAt: step.resultAt },
      );
    }
    return [addressed];
  }
 ...
 */
function extractEventId(body: string): string | undefined {
  try {
...
/** The parts of a Gradio event stream the adapter needs. */
interface SseEvents {
  /** The `complete` event's data payload, when it arrived. */
  readonly complete?: string;
  /** An `error` event's message, when one arrived. */
  readonly error?: string;
  /** How many events were seen, for diagnostics. */
...
function parseSse(body: string): SseEvents {
  ...
  for (const rawLine of body.split(/\r?\n/)) {
    ...
    seen += 1;
    if (current === 'complete' && complete === undefined) complete = payload;
    if (current === 'error' && error === undefined && payload !== 'null' && payload.length > 0) error = payload;
  }
  return {
 ...
 */
async function extractMesh(
  step: ResolvedStep,
  outputs: readonly unknown[],
  config: ResolvedConfig,
  invocation: AdapterInvocation,
  log: AdapterLogger,
): Promise<MeshPayload | undefined> {
  const candidates = collectFileCandidates(outputs, step.resultFormat);
  const preferred =
    candidates.find((candidate) => step.resultFormat !== undefined && candidate.format === step.resultFormat) ??
...
/** A file-like value found in an engine response. */
interface FileCandidate {
  /** What the engine called it; used in messages and as the artifact label. */
  readonly label: string;
  /** A server-side path or a URL. */
  readonly location?: string;
  /** Inline base64 content, when the engine embedded it. */
  readonly base64?: string;
  /** The format implied by the name or the declared content type. */
  readonly format: ThreeDFormat | undefined;
}
...
function collectFileCandidates(
  outputs: readonly unknown[],
  declared: ThreeDFormat | undefined,
  anyFile = false,
): FileCandidate[] {
  ...
  const visit = (value: unknown, label: string, depth: number): void => {
    ...
    for (const [key, entry] of Object.entries(value)) {
      if (key === 'path' || key === 'url' || key === 'name' || key === 'mime_type') continue;
      if (typeof entry === 'string' || Array.isArray(entry) || isRecord(entry)) visit(entry, `${label}.${key}`, depth + 1);
    }
  };
...
async function readCandidate(
  ...
  if (/^https?:\/\//.test(location)) {
    ...
    if (!read.ok) throw engineError(location, read.reason, invocation.model.id);
  ...
  try {
    ...
    if (filePath === undefined) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine reported its result at "${location}", which this machine cannot read ` +
          `(${error instanceof Error ? error.message : String(error)}). ` +
          'If the engine runs in a container or on another host, set adapterConfig.filePath to its file-download route.',
        { modelId: invocation.model.id, location, step: step.index },
      );
    }
    ...
    if (!read.ok) {
      throw new ModelHubError(
        'INVOCATION_FAILED',
        `the engine reported its result at "${location}" but it could not be read locally or fetched from ${url}: ${read.reason}`,
        { modelId: invocation.model.id, location, url, step: step.index },
      );
    }
    return new Uint8Array(Buffer.from(read.body, 'base64'));
  }
...
async function storeMesh(
  ...
  const parameters: Record<string, JsonValue> = { ...config.generationParameters };
  if (config.previewIndex !== undefined) parameters['previewIndex'] = config.previewIndex;
...
async function storePreview(
  ...
  if (config.previewIndex === undefined) return undefined;
  const value = resolvePreviewValue(results, config.previewIndex);
  if (value === undefined) {
    invocation.log.warn(
      `three_d: previewIndex ${config.previewIndex} matched no engine output (${results.flat().length} available)`,
  ...
  try {
    const bytes = /^https?:\/\//.test(location)
      ? await (async (): Promise<Uint8Array | undefined> => {
          const read = await request({
            url: location,
            method: 'GET',
            timeoutMs: config.requestTimeoutMs,
    ...
    invocation.log.warn(
      `three_d: the engine preview at ${location} could not be stored (${error instanceof Error ? error.message : String(error)})`,
      { modelId: invocation.model.id },
    );
    return undefined;
  }
 ...
 */
function resolvePreviewValue(results: readonly unknown[][], index: number): unknown {
...
interface RequestOptions {
  ...
  /** Return the body base64-encoded rather than as text, for binary downloads. */
 ...
 */
async function request(options: RequestOptions): Promise<ResponseRead> {
  ...
  try {
    ...
    if (options.binary === true) {
      if (!response.ok) return { ok: false, status: response.status, reason: `HTTP ${response.status} from ${options.url}`, aborted: false };
 ...
 * @returns the error to throw.
 */
function engineError(url: string, reason: string, modelId: string): ModelHubError {
