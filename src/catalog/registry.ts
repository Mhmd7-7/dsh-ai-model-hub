...
export class ModelCatalog {
  ...
  /**
   * Publish a fresh machine profile.
   *
   * The profile is not a property of the catalog's *models* — it is a property of
   * the machine the catalog is being routed against — but the catalog is what
   * holds it, because {@link checkResources} is what reads it and the router must
   * be able to ask that question synchronously, per candidate, inside one
   * decision. A re-probe therefore replaces the profile in place rather than
   * rebuilding anything: the models, the hosts, and every open reference to this
   * catalog are untouched, and the next routing decision simply sees the new
   * numbers.
   *
   * @param profile - the freshly measured profile.
   */
  replaceMachineProfile(profile: MachineProfile): void {
   ...
   *
   * Two questions are asked, and the `live` flag chooses between them:
   *
   * - **Idle machine** (`live: false`, the default, used for status and for
   *   start-time gating): declared needs against detected *totals*. A model that
   *   does not fit an idle machine can never run here, so this is what produces
   *   the `unsupported` availability state.
   * - **Routing** (`live: true`, used by the router): declared needs against
   *   current *headroom* when the model is already resident, and against totals
   *   when it is not — because starting a stopped model is precisely what makes
   *   the whole machine's memory available to it. A model that is already
   *   running has, by definition, already paid for its memory.
   *
   * The check stays conservative: no overcommit, and a shortfall is a refusal
   * rather than a gamble. Real schedulers multiplex; a hub that lets a 24 GB
   * model onto an 8 GB card produces a confusing engine crash instead of a clear
   * routing rejection.
   *
   * @param model - the model to test.
   * @param options - `live` measures headroom instead of total capacity; `detail`
   *   adds the measured figures to the reason so a rejection explains itself.
   * @returns `{ supported: true }`, or `supported: false` with the shortfall.
   */
  checkResources(
    model: ResolvedModel,
    options: { readonly live?: boolean; readonly detail?: boolean } = {},
    ...
    // Headroom is used only when it is actually known. An absent figure means
    // "never measured", which must not masquerade as "nothing free".
    const vramBudget = live && machine.availableVramGb !== undefined ? machine.availableVramGb : machine.vramGb;
    const ramBudget = live && machine.availableRamGb !== undefined ? machine.availableRamGb : machine.ramGb;
    const vramBasis = live && machine.availableVramGb !== undefined ? 'available' : 'total';
    const ramBasis = live && machine.availableRamGb !== undefined ? 'available' : 'total';
    const total = options.detail === true;
    ...
    if (machine.hasGpu && model.resources.vramGb > vramBudget) {
      return {
        supported: false,
        reason:
          `needs ${model.resources.vramGb} GiB VRAM but only ${vramBudget} GiB is ` +
          `${vramBasis === 'available' ? 'available' : 'present on this machine'}` +
          (total && vramBasis === 'available' ? ` (of ${machine.vramGb} GiB total)` : ''),
      };
    }
    if (ramBudget > 0 && model.resources.ramGb > ramBudget) {
      return {
        supported: false,
        reason:
          `needs ${model.resources.ramGb} GiB RAM but only ${ramBudget} GiB is ` +
          `${ramBasis === 'available' ? 'available' : 'present on this machine'}` +
          (total && ramBasis === 'available' ? ` (of ${machine.ramGb} GiB total)` : ''),
      };
    }
    return { supported: true };
  }
