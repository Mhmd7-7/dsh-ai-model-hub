...
/** Details attached to a hub error, preserved for logs and tool output. */
export type HubErrorDetails = Readonly<Record<string, unknown>>;
...
export class ModelHubError extends Error {
  ...
  readonly details: HubErrorDetails;
   ...
   */
  constructor(code: ErrorCode, message: string, details: HubErrorDetails = {}) {
    super(message);
   ...
   */
  toJSON(): { code: ErrorCode; message: string; details: HubErrorDetails } {
...
export function describeError(error: unknown): {
  ...
  if (error instanceof Error) {
    ...
    if (typeof systemCode === 'string') details['systemCode'] = systemCode;
...
export function toHubError(
  error: unknown,
  fallbackCode: ErrorCode = 'INVOCATION_FAILED',
  context: HubErrorDetails = {},
): ModelHubError {
  if (error instanceof ModelHubError) {
