...
import type { AdapterLogger } from '../adapters/types.ts';
import { AdapterRegistry, silentLogger } from '../adapters/types.ts';
import type { AvailabilityState, HealthReport, LifecycleState, ModelRuntimeStatus } from '../types.ts';
import { ModelHubError, toHubError } from '../errors.ts';
import type { ExecutionPolicy, ManagedProcess } from '../util/process.ts';
import {
...
/** How an invocation gate may treat a model. */
export interface GateDecision {
  /** Whether the caller may proceed. */
  readonly allowed: boolean;
  /** What the runtime did on the way to the decision. */
  readonly coldStart: boolean;
  /** Why the call was refused. */
  readonly reason?: string;
}

/** Options for {@link RuntimeManager}. */
export interface RuntimeManagerOptions {
  /** The catalog of models. */
  readonly catalog: ModelCatalog;
  /** The adapters able to serve them. */
  readonly adapters: AdapterRegistry;
  /** Process-execution policy. Defaults to the shipped allowlist policy. */
  readonly policy?: ExecutionPolicy;
  /** Diagnostic sink. */
  readonly log?: (message: string, fields?: Readonly<Record<string, unknown>>) => void;
  /**
   * How often to re-probe health, in milliseconds. `0` disables periodic probes.
   * Defaults to 30000.
   */
  readonly healthIntervalMs?: number;
  /**
   * How often to evaluate idle timeouts, in milliseconds. `0` disables the
...
interface ModelState {
  ...
  /** Serializes start/stop so two concurrent invocations cannot double-spawn. */
...
export class RuntimeManager {
  ...
  getModelStatus(modelId: string): ModelRuntimeStatus {
    ...
    if (state.health !== undefined) status.health = state.health;
    const pid = state.process?.pid;
    if (pid !== undefined) status.pid = pid;
    if (state.startedAt !== undefined) status.startedAt = state.startedAt;
    if (state.reason !== undefined) status.reason = state.reason;
    return status;
  }
   ...
   */
  getAllStatuses(): readonly ModelRuntimeStatus[] {
   ...
   */
  checkAvailability(modelId: string): AvailabilityState {
  ...
  async probeHealth(modelId: string): Promise<HealthReport> {
    ...
    if (!model.enabled) {
      const report: HealthReport = { healthy: false, checkedAt: Date.now(), detail: 'disabled' };
      state.health = report;
    ...
    try {
      report = await withTimeout(
        this.runHealthCheck(model, controller.signal),
        budget,
        `health check for ${modelId}`,
      );
    } catch (error) {
   ...
   */
  private async probeEndpointLiveness(model: ResolvedModel): Promise<boolean> {
    if (model.runtime.endpoint === undefined) return false;
   ...
   * @returns the probe result.
   */
  private async runHealthCheck(model: ResolvedModel, signal: AbortSignal): Promise<HealthReport> {
    ...
    switch (model.health.kind) {
      ...
      case 'command': {
        ...
        const result = await runCommand(
          { command, args: [...(model.health.args ?? [])] },
          {
      ...
      default: {
        const exhaustive: never = model.health.kind;
    ...
    return this.serializeTransition(state, async () => {
      ...
      if (!model.lifecycle.startable || model.lifecycle.start === undefined) {
        throw new ModelHubError(
          ...
          `model "${modelId}" is not startable (no launch command configured). ` +
            'Start its engine yourself and it will be detected as available.',
          { modelId, endpoint: model.runtime.endpoint },
        );
      }
  ...
  private async awaitHealthy(
    ...
    let last: HealthReport = { healthy: false, checkedAt: Date.now(), detail: 'not probed yet' };
  ...
  private async stopProcess(
    ...
    if (model.lifecycle.stop !== undefined) {
      const result = await runCommand(
        { command: model.lifecycle.stop.command, args: [...(model.lifecycle.stop.args ?? [])] },
        { timeoutMs: Math.max(grace, 5_000), policy: this.policy, label: `stop:${model.id}` },
      );
      if (!result.ok) {
   ...
   */
  async stopIdleModels(): Promise<readonly string[]> {
    if (this.disposed) return [];
    const stopped: string[] = [];
    const now = Date.now();
    for (const model of this.catalog.listModels()) {
      if (model.lifecycle.idleTimeoutMs <= 0) continue;
      const state = this.states.get(model.id);
      if (state === undefined || state.process === undefined) continue;
      if (state.activeInvocations > 0) continue;
      const since = state.lastUsedAt ?? state.startedAt;
      if (since === undefined) continue;
      if (now - since < model.lifecycle.idleTimeoutMs) continue;
      ...
      try {
        ...
        if (result.stopped) stopped.push(model.id);
      } catch (error) {
   ...
   */
  async ensureReady(modelId: string): Promise<GateDecision> {
    const model = this.catalog.requireModel(modelId);
    ...
    if (status.availability === 'unsupported') {
      throw new ModelHubError(
        ...
        `model "${modelId}" cannot run here: ${status.reason ?? 'unsupported'}`,
        { modelId, reason: status.reason },
      );
    }
    ...
    if (!model.lifecycle.startable) {
      ...
      if (probe.healthy) return { allowed: true, coldStart: false };
      throw new ModelHubError(
        'MODEL_UNAVAILABLE',
        `model "${modelId}" is not running and is not startable by the hub` +
          (model.runtime.endpoint === undefined ? '' : ` (expected it at ${model.runtime.endpoint})`),
        { modelId, health: probe.detail, endpoint: model.runtime.endpoint },
      );
    }
    ...
    if (status.availability === 'starting') {
      ...
      if (after.healthy) return { allowed: true, coldStart: true };
   ...
   * @param modelId - the model id.
   */
  beginInvocation(modelId: string): void {
    const state = this.requireState(modelId);
   ...
   * @param modelId - the model id.
   */
  endInvocation(modelId: string): void {
    const state = this.states.get(modelId);
   ...
   * @returns a logger.
   */
  adapterLogger(modelId: string): AdapterLogger {
    return {
   ...
   */
  private serializeTransition<T>(state: ModelState, operation: () => Promise<T>): Promise<T> {
    const run = state.transition.then(operation, operation);
    state.transition = run.then(
      () => undefined,
      () => undefined,
    );
    return run;
 ...
 */
export function parseHostPort(model: ResolvedModel): { host: string; port: number } | undefined {
