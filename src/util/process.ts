...
import type { CommandSpec } from '../catalog/descriptor.ts';
...
/** Limits applied to every spawn. */
export interface ExecutionPolicy {
  /** Executable names permitted without special handling. */
  readonly allowlist: readonly string[];
  /**
   * Whether to permit executables outside {@link allowlist}.
   *
   * Defaults to false. An operator who needs an unusual engine path sets this,
   * which is an explicit, auditable decision rather than a silent widening.
   */
  readonly allowAnyCommand: boolean;
  /** Longest accepted single argument, in characters. Defaults to 4096. */
  readonly maxArgumentLength: number;
  /** Longest accepted argument list. Defaults to 256. */
  readonly maxArgumentCount: number;
  /**
   * Environment variable names removed from every child's environment.
   *
   * Children inherit the operator's environment so engines can find their own
   * configuration, minus anything credential-shaped: a model process has no
   * business holding the harness's API keys.
   */
  readonly scrubEnvPatterns: readonly string[];
}
...
export function withPolicy(base: ExecutionPolicy, override: Partial<ExecutionPolicy>): ExecutionPolicy {
 ...
 */
export function assertSafeArguments(
  ...
  if (args.length > policy.maxArgumentCount) {
    throw new UnsafeCommandError(
      `${context}: refusing to launch with ${args.length} arguments (limit ${policy.maxArgumentCount})`,
      { argumentCount: args.length, limit: policy.maxArgumentCount },
    );
  }
  args.forEach((arg, index) => {
    ...
    if (/[\r\n]/.test(arg)) {
      throw new UnsafeCommandError(
        `${context}: argument ${index} contains a line break, which suggests a shell command was built as a string`,
        { index },
      );
    }
    if (arg.length > policy.maxArgumentLength) {
      throw new UnsafeCommandError(
        `${context}: argument ${index} is ${arg.length} characters (limit ${policy.maxArgumentLength})`,
        { index, length: arg.length, limit: policy.maxArgumentLength },
      );
    }
  });
}
...
export function assertAllowedCommand(
  ...
  if (policy.allowAnyCommand) return;

  const base = commandBasename(command);
  const allowed = policy.allowlist.some((entry) => commandBasename(entry) === base);
  if (!allowed) {
    throw new UnsafeCommandError(
      `${context}: "${command}" is not in the command allowlist. ` +
        `Either use an allowlisted engine (${policy.allowlist.slice(0, 8).join(', ')}, …) ` +
        'or set `allowAnyCommand: true` in the hub configuration after reviewing what it runs.',
      { command, basename: base, allowlist: [...policy.allowlist] },
    );
  }
}
...
export function buildChildEnvironment(
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (scrubbed.some((pattern) => upper.includes(pattern))) continue;
    result[key] = value;
  }
  if (extra !== undefined) {
...
/** The outcome of a one-shot command. */
export interface CommandResult {
  /** Whether the command ran and exited zero. */
  readonly ok: boolean;
  /** Exit code, or `undefined` when the process was killed by a signal. */
  ...
  readonly reason?: 'spawn_failed' | 'timeout' | 'non_zero_exit' | 'aborted';
}

/** Options for a one-shot command. */
export interface RunCommandOptions {
  /** Kill the process after this many milliseconds. Defaults to 10000. */
  readonly timeoutMs?: number;
  /** Cancellation from the caller. */
  readonly signal?: AbortSignal;
  /** Cap on captured output per stream, in characters. Defaults to 65536. */
  readonly maxOutputChars?: number;
  /** Policy to enforce. Defaults to {@link DEFAULT_EXECUTION_POLICY}. */
  readonly policy?: ExecutionPolicy;
  /** Extra environment variables. */
  readonly env?: Readonly<Record<string, string>>;
  /** Label used in errors and logs. Defaults to the command name. */
  ...
  return new Promise<CommandResult>((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      finish({
        ...
        stdout: truncatedOut,
        stderr: truncatedErr,
        ...(code === 0 ? {} : { reason: 'non_zero_exit' as const }),
      });
    });
  });
}

/** A handle on a long-lived child process. */
export interface ManagedProcess {
  /** OS process id, when the platform provided one. */
  readonly pid: number | undefined;
  /** Whether the process is still running. */
  readonly running: boolean;
  /** Bounded recent stderr, for diagnosing a failed start. */
  stderrSnapshot(): string;
  /** Bounded recent stdout, for diagnosing a failed start. */
  stdoutSnapshot(): string;
  /**
   * Ask the process to stop, escalating to a forceful kill after a grace period.
   * @param graceMs - how long to wait for a clean exit before killing.
   * @returns whether the process is gone when the call resolves.
   */
 ...
 */
export function spawnProcess(spec: CommandSpec, options: RunCommandOptions = {}): ManagedProcess {
  const policy = options.policy ?? DEFAULT_EXECUTION_POLICY;
  const label = options.label ?? spec.command;
 ...
 */
async function terminateTree(child: ReturnType<typeof spawn>, graceMs: number): Promise<void> {
  const pid = child.pid;
...
export function withTimeout<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      settle(() =>
        reject(
          new ModelHubError('INVOCATION_TIMEOUT', `${label} exceeded its ${timeoutMs} ms budget`, {
            timeoutMs,
          }),
        ),
      );
