...
import { DEFAULT_EXECUTION_POLICY, runCommand, withPolicy } from './util/process.ts';
import type { ExecutionPolicy } from './util/process.ts';
import type { GpuInfo, MachineProfile } from './types.ts';
...
/** Options for {@link probeMachine}. */
export interface MachineProbeOptions {
  /** Skip the (slow) GPU probe and report only OS memory. */
  readonly skipGpuProbe?: boolean;
  /** Skip the filesystem probe. Disk is advisory, so this is safe to skip. */
  readonly skipDiskProbe?: boolean;
  /** Directory whose filesystem is measured. Defaults to the working directory. */
  readonly diskPath?: string;
  /** Per-probe timeout in milliseconds. Defaults to 5000. */
  readonly timeoutMs?: number;
  /**
   * Execution policy the deployment is already running under.
   *
   * Accepted so an operator who has *narrowed* the allowlist for their own
   * engines does not silently lose GPU detection to it. The probe widens the
   * policy it is given by exactly one binary — see {@link GPU_PROBE_COMMAND} —
   * because `nvidia-smi` is a fixed diagnostic query, not a model launch.
   */
 ...
 */
export async function probeMachine(options: MachineProbeOptions = {}): Promise<MachineProbeResult> {
  ...
  let availableDiskGb: number | undefined;
...
): Promise<{ gpus: readonly GpuInfo[]; detail: string } | undefined> {
 ...
 */
async function probeAvailableRam(timeoutMs: number): Promise<number | undefined> {
  if (process.platform === 'linux') {
    const available = await readMemAvailableKb(timeoutMs);
    if (available !== undefined) return round1((available * 1024) / GIB);
  }
  try {
    const free = freemem() / GIB;
    if (!Number.isFinite(free) || free < 0) return undefined;
    return round1(free);
  } catch {
    return undefined;
  }
 ...
 */
async function readMemAvailableKb(_timeoutMs: number): Promise<number | undefined> {
  try {
 ...
 */
async function probeDisk(path: string): Promise<{ availableGb: number; path: string } | undefined> {
  try {
