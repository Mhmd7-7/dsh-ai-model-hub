...
export const THREE_D_FORMATS = ['glb', 'gltf', 'obj', 'stl', 'ply'];
 ...
 */
export function threeDFormatOf(value) {
    ...
    for (const format of THREE_D_FORMATS) {
        ...
        if (bare === format || bare === info.mimeType || normalized === info.mimeType)
            return format;
    }
    ...
    for (const format of THREE_D_FORMATS) {
        const info = FORMATS[format];
        if (tail === format || tail.endsWith(`.${format}`) || tail.includes(format))
            return format;
        if (info.mimeType.endsWith(`/${tail}`))
            return format;
    }
    return undefined;
}
 ...
 */
export function threeDFormatInfo(format) {
 ...
 */
export function sniffThreeDFormat(bytes, claimed) {
...
function detectFormat(bytes) {
    ...
    if (looksLikeObj(text))
        return 'obj';
    if (text.startsWith('solid') && text.includes('facet'))
        return 'stl';
 ...
 */
function decodeHead(bytes, maxBytes) {
 ...
 */
export function measureThreeD(bytes, format) {
    const result = {};
    if (format === 'obj') {
 ...
 */
function readGltfAccessors(bytes, format) {
    ...
    try {
        ...
        return Array.isArray(accessors) ? accessors : undefined;
    }
    catch {
        return undefined;
    }
