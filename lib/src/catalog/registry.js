...
export class ModelCatalog {
    ...
    checkResources(model, options = {}) {
        ...
        if (machine.hasGpu && model.resources.vramGb > vramBudget) {
            return {
                supported: false,
                reason: `needs ${model.resources.vramGb} GiB VRAM but only ${vramBudget} GiB is ` +
        ...
        if (ramBudget > 0 && model.resources.ramGb > ramBudget) {
            return {
                supported: false,
                reason: `needs ${model.resources.ramGb} GiB RAM but only ${ramBudget} GiB is ` +
