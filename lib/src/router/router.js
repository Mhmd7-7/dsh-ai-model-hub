 ...
 */
import { describeArtifact } from "../artifacts/types.js";
import { IO_TYPES, isIoType } from "../catalog/capabilities.js";
 ...
 */
export async function resolveRequestInputs(context, request) {
    const inputs = [];
    ...
    for (const entry of declared) {
        ...
        if (artifact === undefined) {
            throw new ModelHubError('ARTIFACT_ERROR', `input artifact "${reference.id}" does not exist. ` +
                'Artifact ids come from a previous invocation result or from list_artifacts.', { artifactId: reference.id, capability: request.capability });
        }
        if (reference.type !== undefined && reference.type.length > 0) {
            ...
            if (!isIoType(reference.type)) {
                throw new ModelHubError('ARTIFACT_ERROR', `input artifact "${artifact.id}" was declared as type "${reference.type}", which is not an artifact kind`, { artifactId: artifact.id, declaredType: reference.type, knownKinds: IO_TYPES });
            }
            if (reference.type !== artifact.type) {
                throw new ModelHubError('ARTIFACT_ERROR', `input artifact "${artifact.id}" is of type "${artifact.type}" but was declared as "${reference.type}"`, { artifactId: artifact.id, declaredType: reference.type, actualType: artifact.type });
            }
 ...
 */
function normalizeArtifactInput(entry) {
 ...
 */
export function routeRequest(context, request, policy = DEFAULT_ROUTING_POLICY) {
    ...
    if (eligible.length === 0) {
        ...
        throw new ModelHubError('NO_COMPATIBLE_MODEL', buildNoCandidateMessage(capability, considered, offered.length, request), {
            capability,
 ...
 */
function evaluateCandidate(model, request, inputs) {
    ...
    switch (inputs.availability) {
        ...
        default: {
            const exhaustive = inputs.availability;
 ...
 */
function buildNoCandidateMessage(capability, candidates, declaringCount, request) {
    const lines = [`no model can serve capability "${capability}" for this request.`];
    if (declaringCount === 0) {
        lines.push(`No configured model declares "${capability}" at all. Add one to config/models.json, ` +
 ...
 */
export function describeResolvedRequest(request) {
 ...
 * @returns a multi-line explanation.
 */
export function explainDecision(decision, verbose = false) {
