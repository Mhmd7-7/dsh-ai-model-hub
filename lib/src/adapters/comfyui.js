...
async function loadWorkflowFile(path) {
    ...
    try {
        ...
        throw new ModelHubError('CONFIG_ERROR', `comfyui workflow file "${absolute}" could not be read: ${error instanceof Error ? error.message : String(error)}`, { path: absolute });
    }
    let parsed;
    try {
        ...
        throw new ModelHubError('CONFIG_ERROR', `comfyui workflow file "${absolute}" is not valid JSON: ${error instanceof Error ? error.message : String(error)}`, { path: absolute });
    }
