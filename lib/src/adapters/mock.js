...
import { delay } from "../util/process.js";
import { readArtifactConventions } from "../artifacts/types.js";
import { MOCK_STL_VERTEX_COUNT, renderMockPng, renderMockStl } from "./png.js";
const MOCK_DEFAULTS = {
    latencyMs: 0,
    width: 512,
    height: 512,
 ...
 */
function numberSetting(options, config, key, fallback) {
 ...
 */
async function simulateWork(ms, signal) {
    if (ms <= 0) {
        if (signal.aborted)
            throw new ModelHubError('INVOCATION_ABORTED', 'invocation cancelled before it started');
        return;
    }
    if (signal.aborted)
        throw new ModelHubError('INVOCATION_ABORTED', 'invocation cancelled before it started');
    const cancelled = new Promise((_resolve, reject) => {
        signal.addEventListener('abort', () => reject(new ModelHubError('INVOCATION_ABORTED', 'invocation cancelled during simulated work')), { once: true });
    });
 ...
 */
function requirePrompt(invocation, required) {
    ...
    return {
        ...
        async invoke(invocation) {
            ...
            if (handler === undefined) {
                throw new ModelHubError('UNSUPPORTED_OPERATION', `the mock adapter does not implement capability "${invocation.capability}"`, { capability: invocation.capability, modelId: invocation.model.id });
            }
...
const HANDLERS = {
    text_to_text: async (invocation) => {
        ...
        const contextNote = invocation.inputs.length === 0
            ? ''
    ...
    image_to_image: async (invocation, settings) => {
        ...
        if (source === undefined) {
            throw new ModelHubError('INVOCATION_FAILED', 'image_to_image requires an input artifact of type `image`', { modelId: invocation.model.id, providedTypes: invocation.inputs.map((artifact) => artifact.type) });
        }
    ...
    image_to_3d: async (invocation) => {
        const source = invocation.inputs.find((artifact) => artifact.type === 'image');
        if (source === undefined) {
            throw new ModelHubError('INVOCATION_FAILED', 'image_to_3d requires an input artifact of type `image`', { modelId: invocation.model.id, providedTypes: invocation.inputs.map((artifact) => artifact.type) });
        }
        const conventions = readArtifactConventions(source);
    ...
    image_understanding: async (invocation) => {
        const source = invocation.inputs.find((artifact) => artifact.type === 'image');
        if (source === undefined) {
            throw new ModelHubError('INVOCATION_FAILED', 'image_understanding requires an input artifact of type `image`', { modelId: invocation.model.id, providedTypes: invocation.inputs.map((artifact) => artifact.type) });
        }
        const conventions = readArtifactConventions(source);
        const text = [
    ...
    audio_generation: async (invocation) => {
        ...
        const seconds = Math.min(numberSetting(invocation.options, invocation.model.adapterConfig, 'seconds', 1), invocation.model.limits.maxDurationSeconds ?? Number.POSITIVE_INFINITY);
    ...
    speech_to_text: async (invocation) => {
        ...
        if (source === undefined) {
            throw new ModelHubError('INVOCATION_FAILED', 'speech_to_text requires an input artifact of type `audio`', { modelId: invocation.model.id, providedTypes: invocation.inputs.map((artifact) => artifact.type) });
        }
    ...
    video_generation: async (invocation, settings) => {
        ...
        const seconds = Math.min(numberSetting(invocation.options, invocation.model.adapterConfig, 'seconds', 2), invocation.model.limits.maxDurationSeconds ?? Number.POSITIVE_INFINITY);
        const manifest = JSON.stringify({ fixture: 'video_generation', prompt, seconds, width: settings.width, height: settings.height, frames: Math.round(seconds * 8) }, null, 2);
        const artifact = await invocation.artifacts.put({
            type: 'video',
            text: manifest,
 ...
 */
export function renderMockWav(sampleCount, sampleRate, seed) {
