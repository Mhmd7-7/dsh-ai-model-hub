 ...
 */
async function resolveConfig(model, catalogDir) {
    const where = `model "${model.id}" (adapter ${model.adapter})`;
 ...
 */
async function readStepsFile(model, config, where, catalogDir) {
    ...
    try {
        ...
        throw configError(`${where}: could not read the 3D step declaration ${describeAdapterPath(resolved)}: ${error instanceof Error ? error.message : String(error)}`, { modelId: model.id, stepsPath: file, base: resolved.base ?? null, origin: resolved.origin });
    }
    let parsed;
    try {
        ...
        throw configError(`${where}: the 3D step declaration ${describeAdapterPath(resolved)} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`, { modelId: model.id, stepsPath: file, base: resolved.base ?? null, origin: resolved.origin });
    }
    if (!isRecord(parsed)) {
...
export function createThreeDAdapter() {
    return {
         ...
         */
        async health(model, signal, context) {
