 ...
 */
export function renderMockPng(width, height, seed, options = {}) {
 ...
 */
export function renderMockStl(seed, options = {}) {
    let hash = 0x811c9dc5;
    for (let index = 0; index < seed.length; index += 1) {
        hash ^= seed.charCodeAt(index);
        hash = Math.imul(hash, 0x01000193) >>> 0;
    ...
    const lines = [`solid ai_model_hub_mock_${seed.replace(/[^\w-]/g, '_').slice(0, 40)}`];
