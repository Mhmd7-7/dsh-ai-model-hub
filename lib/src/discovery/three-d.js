...
import { isValidModelId } from "../catalog/descriptor.js";
import { fetchJson, isRecordLike, readArray, readString, slugifyModelId } from "./http.js";
...
export const THREE_D_ENGINE_ALIASES = [
    '3d',
    'threed',
    'three-d',
    'trellis',
...
const DISCOVERED_TAGS = ['local', 'discovered', THREE_D_ENGINE];
/**
 ...
 */
const ENDPOINT_CAPABILITY_PATTERNS = [
 ...
 */
export function parseGradioSurface(raw, configPath) {
    if (!isRecordLike(raw))
        return undefined;
    const names = new Set();
    ...
    for (const endpoint of endpoints) {
        ...
        if (EXPORT_ENDPOINT_PATTERN.test(endpoint))
            exportOnly.push(endpoint);
    }
    return {
        configPath,
        endpoints,
        capabilities,
        exportOnly,
        ...(readString(raw, 'version') === undefined ? {} : { version: readString(raw, 'version') }),
    };
}
/**
 ...
 */
export function parseThreeDHostConfig(host) {
    ...
    for (const [index, entry] of source.entries()) {
        ...
        if (id === undefined)
            return { ok: false, reason: `models[${index}].id is required` };
        if (!isValidModelId(id)) {
            return { ok: false, reason: `models[${index}].id "${id}" is not lowercase kebab-case` };
        }
        if (seen.has(id))
            return { ok: false, reason: `models[${index}].id "${id}" is declared twice` };
        seen.add(id);
 ...
 */
function readCapabilityList(raw) {
    if (raw === undefined)
        return [];
 ...
 */
export function mapThreeDModel(model, surface, host) {
    const capabilities = model.capabilities === undefined || model.capabilities.length === 0
    ...
    if (capabilities.length === 0) {
        ...
        return {
            ok: false,
            reason: `model "${model.id}" declares ${declared} but ${host.runtime.engine} at ${host.runtime.endpoint} ` +
                `only exposes routes for ${proven}`,
        };
    }
    const { inputTypes, outputTypes } = ioForCapabilities(capabilities);
    const resources = {};
    if (model.vramGb !== undefined)
        resources.vramGb = model.vramGb;
    if (model.ramGb !== undefined)
        resources.ramGb = model.ramGb;
    if (model.requiresGpu !== undefined)
        resources.requiresGpu = model.requiresGpu;
    ...
    return {
        ...
        descriptor: {
            ...
            notes: `Discovered from the ${host.runtime.engine} API surface (${surface.endpoints.length} endpoint(s)); ` +
 ...
 */
function primaryApiFor(capabilities, surface) {
    const order = ['image_to_3d', 'text_to_3d'];
    for (const capability of order) {
        if (!capabilities.includes(capability))
            continue;
        for (const endpoint of surface.endpoints) {
            const matched = ENDPOINT_CAPABILITY_PATTERNS.some((entry) => entry.capability === capability && entry.pattern.test(endpoint));
            if (matched)
                return endpoint;
        }
    }
 ...
 */
export function createThreeDDiscoverer(options = {}) {
    ...
    return {
        ...
        async discover(host, signal) {
            ...
            if (surface === undefined) {
                throw new Error(`could not describe the 3D engine at ${endpoint}. Tried ${GRADIO_CONFIG_PATHS.join(' and ')} — ` +
            ...
            if (declarations.length === 0) {
                ...
                if (implied.ok)
                    descriptors.push(implied.descriptor);
                else
                    skipped.push(implied.reason);
            }
            else {
            ...
            if (descriptors.length === 0) {
                const exportNote = surface.exportOnly.length > 0
                ...
                throw new Error(`${endpoint} is reachable but exposes no image-to-3D or text-to-3D route, so no 3D model can be published.` +
                    exportNote +
                    skipNote);
            }
            return descriptors;
 ...
 */
async function readSurface(endpoint, signal, timeoutMs) {
