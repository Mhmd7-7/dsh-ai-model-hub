 ...
 */
export async function fetchJson(url, signal, timeoutMs, init = {}) {
    ...
    try {
        ...
        if (controller.signal.aborted)
            return { ok: false, reason: `request to ${url} was aborted or timed out` };
 ...
 */
export function readNumberBySuffix(value, suffix) {
    if (!isRecord(value))
        return undefined;
 ...
 */
export function hasKeyMatching(value, predicate) {
    if (!isRecord(value))
        return false;
 ...
 */
export function slugifyModelId(name, prefix, fallback = 'model') {
