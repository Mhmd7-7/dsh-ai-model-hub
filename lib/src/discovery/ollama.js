 ...
 */
import { bytesToGib, fetchJson, hasKeyMatching, isRecordLike, readArray, readNumber, readNumberBySuffix, readString, slugifyModelId, stableDigest, } from "./http.js";
...
const DISCOVERED_TAGS = ['local', 'discovered', OLLAMA_ENGINE];
/**
 ...
 */
export function parseOllamaTags(raw) {
    const models = [];
 ...
 */
function readDisplayName(entry) {
 ...
 */
export function parseOllamaShow(raw, name = '') {
    ...
    const vision = hasKeyMatching(raw, (key) => /projector|vision|clip/i.test(key)) ||
 ...
 */
export function capabilitiesForOllamaModel(candidate) {
    const capabilities = ['text_to_text'];
    if (candidate.details?.vision === true)
 ...
 */
export function mapOllamaModel(candidate, host) {
 ...
 */
function describeOllamaModel(candidate) {
    const facts = ['Discovered from the Ollama API; not listed in models.json.'];
    const { family, parameterSize, quantizationLevel } = candidate.summary;
    if (family !== undefined)
        facts.push(`family ${family}.`);
    if (parameterSize !== undefined)
        facts.push(`${parameterSize} parameters.`);
    if (quantizationLevel !== undefined)
        facts.push(`quantized ${quantizationLevel}.`);
 ...
 */
export function createOllamaDiscoverer(options = {}) {
    ...
    return {
        ...
        async discover(host, signal) {
            ...
            const candidates = await mapWithConcurrency(summaries, SHOW_CONCURRENCY, async (summary) => {
                const details = await readShow(endpoint, summary.name, signal, requestTimeoutMs);
                if (details === undefined)
                    return { summary, detailSource: 'unreadable' };
 ...
 */
async function readShow(endpoint, name, signal, timeoutMs) {
