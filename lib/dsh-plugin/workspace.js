 ...
 */
export function sessionCwdOf(runContext) {
 ...
 */
export function createArtifactRootResolver(ctx, configuredRoot, log) {
        ...
        if (policy !== undefined && session !== undefined) {
            try {
                ...
                if (typeof root === 'string' && isAbsolute(root))
                    return join(root, ARTIFACTS_DIRECTORY);
            }
            catch (error) {
                ...
                if (!warned) {
                    warned = true;
                    log.warn(`could not resolve the sandbox workspace for this call (${error instanceof Error ? error.message : String(error)}); falling back to the session cwd`);
                }
            }
        }
