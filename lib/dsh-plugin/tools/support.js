 ...
 */
import { describeArtifact, toHubError } from "../../src/index.js";
/**
...
export function formatAvailability(status) {
    const health = status.health === undefined
        ? ''
 ...
 */
export function formatModel(view, indent = '') {
 ...
 */
export function formatResources(vramGb, ramGb, requiresGpu) {
    const parts = [];
    if (vramGb > 0)
        parts.push(`${vramGb} GiB VRAM`);
    if (ramGb > 0)
        parts.push(`${ramGb} GiB RAM`);
    if (requiresGpu)
        parts.push('GPU required');
 ...
 */
export function formatLifecycleControl(startable, stoppable) {
    if (!startable)
        return 'external â€” the hub will not start or stop it';
 ...
 */
export function formatArtifact(artifact, indent = '  ') {
    ...
    if (metadata.length > 0) {
        lines.push(`${indent}  metadata: ${metadata.map(([key, value]) => `${key}=${String(value)}`).join(', ')}`);
    }
 ...
 */
export function toToolError(error, fallbackCode = 'INVOCATION_FAILED', context = {}) {
