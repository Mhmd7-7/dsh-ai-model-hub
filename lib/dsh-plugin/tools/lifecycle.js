...
import { textBlock, toToolError } from "./support.js";
/**
 ...
 */
export function registerLifecycleTools(ctx, service, options) {
    const hub = service.hub;
    ctx.tools.register(defineTool({
        name: 'start_model',
        description: 'Start a local AI model so it can serve requests. ' +
        ...
        execute: async (args) => {
            if (!options.allowProcessLaunch) {
                throw new Error('Error [UNSAFE_OPERATION]: this deployment has disabled hub-managed process launching ' +
    ...
    ctx.tools.register(defineTool({
        name: 'stop_model',
        description: 'Stop a model the hub started, freeing its memory and GPU. ' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Probe a model right now and report whether it is responding, how long the probe took, and why it failed if it did. ' +
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                for (const result of value.results) {
                    ...
                    lines.push(`  ${result.modelId}: ${result.healthy ? `healthy${latency}` : `unhealthy â€” ${result.detail ?? 'no detail'}`} (now ${result.availability})`);
                }
                return textBlock(lines.join('\n'));
            },
        },
        execute: async (args) => {
            try {
                const ids = args.modelId === undefined || args.modelId.trim().length === 0
