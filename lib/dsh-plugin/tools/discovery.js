 ...
 */
export function registerRefreshDiscoveryTool(ctx, service) {
    const hub = service.hub;
    ctx.tools.register(defineTool({
        name: 'refresh_model_discovery',
        description: 'Re-read every configured model engine and republish the catalog, so models installed since the last check ' +
        ...
        output: {
            ...
            render: (_args, value) => {
                if (!value.enabled) {
                    return textBlock('Runtime model discovery is disabled in this deployment, so the catalog is exactly the configured ' +
                ...
                if (value.added.length > 0)
                    lines.push(`Newly usable: ${value.added.join(', ')}`);
                if (value.removed.length > 0)
                    lines.push(`No longer reported: ${value.removed.join(', ')}`);
